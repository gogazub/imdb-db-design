...
for _ in range(10000):
    while True:  # Повторяем, пока вставка не будет успешной
        try:
            email = fake.email()
            cur.execute("""
                INSERT INTO Users (username, email, password_hash, reviews_count)
                VALUES (%s, %s, %s, %s);
            """, (fake.user_name(), email, fake.sha256(), random.randint(0, 100)))
            break  # Выходим из цикла при успешной вставке
        except psycopg2.errors.UniqueViolation:
            # Генерируем новый email, если произошёл конфликт
            print(f"Duplicate email: {email}. Generating a new one...")
            conn.rollback()  # Откатываем транзакцию для следующей попытки
...
# Title
cur.execute("TRUNCATE TABLE Title RESTART IDENTITY CASCADE;")

for _ in range(5000):
    # Генерация названия фильма
    if random.random() < 0.15:  # 30% вероятность добавления цифр
    ...
    rate_count = random.randint(10, 1000)
    poster = fake.url()
    ...
    """, (name, description, slogan, budget, box_office, rating, rate_count, poster, age_restriction))
...
# Генерация 20 000 уникальных отзывов
...
# Фиксация транзакции
conn.commit()
print("Таблица Reviews успешно заполнена 20 000 уникальными записями.")
...
while len(existing_combinations) < 20000:
    ...
    combination = (title_id, user_id)
    if combination not in existing_combinations:  # Проверка уникальности пары
        rating_value = round(random.uniform(1, 10), 1)  # Рейтинг от 1 до 10 с шагом 0.1
